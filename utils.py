...
def defFieldFromAffineMatrix(aff_mat, num_pix_x, num_pix_y):
 ...
 INPUTS:
   aff_mat - a 3 x 3 matrix representing the 2D affine transformation in
 ...
 OUTPUTS:
   def_field - the 2D deformation field
...
def resampImageWithDefFieldPushInterp(source_img, def_field, interp_method = 'linear'):
 ...
 INPUTS:
   source_img - the source image to be resampled, as a 2D matrix
   def_field - the deformation field, as a 3D matrix
   interP_method - any of the interpolation methods accepted by griddat
       function ('linear', 'nearest', or 'cubic')
       default = 'linear'
 ...
 NOTES:
   the deformation field should be a 3D matrix, where the size of the
   first two dimensions is the size of the source image, and the size of
   the 3rd dimension is 2. def_field(:,:,1) contains the x coordinates of
   the transformed pixels, def_field(:,:,2) contains the y coordinates of
